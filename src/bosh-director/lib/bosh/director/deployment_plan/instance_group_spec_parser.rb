  ...
  module DeploymentPlan
    class InstanceGroupSpecParser
      ...
      def parse(options = {})
        ...
        vm_type, vm_resources = parse_vm_type(name)
        ...
        networks = InstanceGroupNetworksParser.new(Network::REQUIRED_DEFAULTS, Network::OPTIONAL_DEFAULTS).parse(@instance_group_spec, name, @deployment.networks)
      ...
      def parse_desired_instances(name, networks)
        ...
        networks.each do |network|
          ...
          if static_ips && static_ips.size != instances
            ...
            "Instance group '#{name}' has #{instances} instances but was allocated #{static_ips.size} static IPs in network '#{network.name}'"
          end
        end
      ...
      def parse_instance_states(name)
        ...
        instance_states.each_pair do |index_or_id, state|
          unless InstanceGroup::VALID_STATES.include?(state)
            ...
            "Invalid state '#{state}' for '#{name}/#{index_or_id}', valid states are: #{InstanceGroup::VALID_STATES.join(', ')}"
          end
        end
          ...
          unless az.nil?
            unless availability_zones.to_a.map(&:name).include?(az)
              raise DeploymentInvalidMigratedFromJob,
                    "Instance group '#{name}' specified for migration to instance group '#{instance_group_name}' refers to availability zone '#{az}'. " \
      ...
      def parse_env
        ...
        if Config.remove_dev_tools
          env.spec['bosh'] ||= {}
          unless env.spec['bosh'].key?('remove_dev_tools')
            env.spec['bosh']['remove_dev_tools'] = Config.remove_dev_tools
          end
        end
      ...
      def update_tmpfs_properties(env, value)
        ...
        unless env.spec['bosh']['job_dir'].key?('tmpfs')
          env.spec['bosh']['job_dir']['tmpfs'] = value
        end

        env.spec['bosh']['agent'] ||= {}
        env.spec['bosh']['agent']['settings'] ||= {}
        unless env.spec['bosh']['agent']['settings'].key?('tmpfs')
          env.spec['bosh']['agent']['settings']['tmpfs'] = value
        end
      end
    end
  end
