      ...
      def find_network(deployment, cidr_ip, network_name, instance_model)
        ...
        if network_subnets_need_checking(network_match_on_name) # manual and global vip
          ...
          networks.reject { |n| n.is_a? DynamicNetwork }.each do |network|
            ...
            if ip_in_subnet
              @logger.debug("Registering existing reservation with IP '#{format_ip(cidr_ip)}' for instance '#{instance_model}' on network '#{network.name}'")
              return network
            end
          end
        elsif network_match_on_name # dynamic and static vip
          @logger.debug("Registering existing reservation with IP '#{format_ip(cidr_ip)}' for instance '#{instance_model}' on network '#{network_name}'")
          return network_match_on_name
        end

        @logger.debug("Failed to find network #{network_name} or a network with valid subnets for #{format_ip(cidr_ip)}, reservation will be marked as obsolete")
