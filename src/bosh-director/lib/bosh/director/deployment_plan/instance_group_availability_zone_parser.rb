    ...
    class InstanceGroupAvailabilityZoneParser
        ...
        networks.each do |network|
          unless network.has_azs?(az_names)
            raise JobNetworkMissingRequiredAvailabilityZone,
              "Instance group '#{instance_group_name}' must specify availability zone that matches availability zones of network '#{network.name}'"
          end
        end
      end
      ...
      def check_validity_of(az_names, instance_group_name)
        ...
        az_names.each do |name|
          unless name.is_a?(String)
            raise JobInvalidAvailabilityZone, "Instance group '#{instance_group_name}' has invalid availability zone '#{name}', string expected"
          end
        end
      end
      ...
      def look_up_from_deployment(az_names, deployment, instance_group_name)
        az_names.map do |name|
          ...
          if az.nil?
            raise JobUnknownAvailabilityZone, "Instance group '#{instance_group_name}' references unknown availability zone '#{name}'"
          end
          az
        end
      end
