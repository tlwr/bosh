...
describe 'restart command', type: :integration do
  ...
  xcontext 'when attempting to restart an errand instance' do
    before do
    ...
    it 'fails gracefully with a useful message' do
      ...
      expect(output).to include('Restart can not be run on instances of type errand. Try the bosh run-errand command.')
    end
  end

  context 'after a successful deploy' do
    let (:instance) { director.instance('foobar', '0') }
    ...
    def started_count
      instance.read_file('started_at').split('\n').count
    end

    def stopped_count
      instance.read_file('stopped_at').split('\n').count
    end

    context 'with an index' do
      it 'restarts the indexed job' do
        expect(started_count).to eq(1)
        expect(File).not_to exist(instance.file_path('stopped_at'))

        output = isolated_restart(instance_group: 'foobar', index: 0)
        expect(output).to match(/Restarting instance foobar: foobar.* \(0\)/)

        expect(started_count).to eq(2)
        expect(stopped_count).to eq(1)


        expect(job_states).to eq(
            'foobar/0' => 'running',
            'foobar/1' => 'running',
            'foobar/2' => 'running',
        )
